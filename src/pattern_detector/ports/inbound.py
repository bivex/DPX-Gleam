"""Inbound driving ports for DPX-Gleam."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Gleam scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".gleam"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Gleam source file."""
