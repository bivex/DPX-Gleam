"""Outbound driven ports for DPX-Gleam."""

from __future__ import annotations

from typing import Protocol
from pattern_detector.domain.code_model import CodeModel, GleamFile
from pattern_detector.domain.detection import DetectionReport


class SourceProviderPort(Protocol):
    """Port for discovering and reading Gleam source files (.gleam)."""
...
class ParserPort(Protocol):
    """Port for parsing Gleam source text into CodeModel."""

    def parse_file(self, file_path: str, content: str) -> GleamFile:
        """Parse a single Gleam file into GleamFile model."""
        ...

    def parse_codebase(self, files: list[tuple[str, str]], target_path: str = "") -> CodeModel:
        """Parse multiple Gleam files into an aggregated CodeModel."""
        ...
