"""Typer and Rich CLI interface for DPX-Gleam."""
...
app = typer.Typer(
    name="dpx-gleam",
    help="⭐ Type-Safe Gleam Static Analyzer: OTP Actors, Algebraic Data Types, Railway Monads, GoF 23 & BEAM Concurrency Pattern Detector.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Gleam source file (.gleam) or directory containing a Gleam project.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'gleam_otp_actor_process', 'result_monad_railway').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e build -e dev).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Gleam codebase for OTP actors, custom types, Railway error flows, GoF patterns, and hazards."""
...
@app.command()
def rules() -> None:
    """List all available Gleam detection rules and quality checks."""
    table = Table(
        title="⭐ DPX-Gleam: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'gleam_otp_actor_process', 'result_monad_railway').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Gleam pattern."""
    ...
    panel = Panel.fit(
        ...
        f"[bold white]Gleam Target:[/bold white] {pdef.gleam_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Adheres to standard Gleam functional idiom.'}",
        title=f"🔍 Gleam Architecture Encyclopedia: {pdef.name}",
