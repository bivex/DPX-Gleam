"""Interactive HTML Observability HUD Formatter for Gleam."""
...
CATEGORY_COLORS: dict[PatternCategory, dict[str, str]] = {
    PatternCategory.GLEAM_IDIOMATIC: {
        "text": "#ff79c6",
        "bg": "#ff79c622",
        "border": "#ff79c6",
        "accent": "#ffaff3",
        "name": "Gleam Idiomatic & Types",
    },
    PatternCategory.OTP_BEAM_CONCURRENCY: {
        "text": "#bd93f9",
        "bg": "#bd93f922",
        "border": "#bd93f9",
        "accent": "#d6b4fc",
        "name": "OTP & BEAM Concurrency",
    },
    PatternCategory.FUNCTIONAL_COMPOSITION: {
        "text": "#8be9fd",
        "bg": "#8be9fd22",
        "border": "#8be9fd",
        "accent": "#a1efff",
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders standalone, responsive HTML dashboard for Gleam DetectionReport with AI Prompt Generator."""
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            finding_prompt = (
                f"Refactor and optimize the following Gleam architectural finding:\n"
                ...
                f"Please provide an idiomatic Gleam 1.x type-safe refactoring leveraging OTP actors, Custom Types / ADTs, Result monads, or 'use <-' continuation syntax."
            )
...
<head>
    ...
    <title>Pattern Scanner Report - DPX-Gleam Architecture HUD - {html.escape(report.project_path or "Codebase")}</title>
    <style>
        :root {{
            --bg: #0f111a;
            --card-bg: #1a1c29;
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, monospace, sans-serif; }}
        ...
        .btn-ai-context {{
            ...
            align-items: center;
            gap: 8px;
            box-shadow: 0 4px 14px rgba(255, 121, 198, 0.35);
        ...
        .btn-view-prompt:hover {{
            border-color: var(--gleam-pink);
            color: var(--gleam-pink);
        }}
        ...
        .btn-all {{ background: #252a3d; color: #f0f6fc; border-color: #3d4461; }}
        ...
        .search-bar:focus {{ border-color: var(--gleam-pink); }}
        ...
        .pattern-card:hover {{ border-color: #ff79c688; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }}
        .card-header {{ background: #222638; padding: 12px 18px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid var(--border); }}
        ...
        code {{ background: #131520; padding: 3px 6px; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; color: #ffaff3; border: 1px solid #2d3142; }}
        ...
        .location-tag {{ color: #ffaff3; font-size: 11px; margin-left: 6px; }}
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>⭐ DPX-Gleam Architecture & OTP Observability HUD</h1>
                <div class="subtitle">Type-Safe Functional Architecture, OTP Actors, Result Monad Railway & GoF 23 • Target: <code>{html.escape(report.project_path or "Target Repository")}</code></div>
            </div>
        ...
        <input type="text" id="searchInput" class="search-bar" placeholder="🔎 Instant search by pattern name, category, target type, or rule (e.g. actor, result, pipe, opaque, adt)...">
    ...
    <div id="promptModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title">🤖 AI Architect Context Prompt (Gleam)</div>
                <button class="modal-close" onclick="closePromptModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 13px; color: #94a3b8; margin-bottom: 12px;">Structured XML/Markdown context for LLMs (Claude, GPT-4, Gemini) including OTP actors, ADT sum types, and Railway error handling analysis.</p>
    ...
    <script>
        ...
        filterBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                btn.style.outline = '2px solid #ff79c6';
                filterCards();
            }});
        }});
    </script>
