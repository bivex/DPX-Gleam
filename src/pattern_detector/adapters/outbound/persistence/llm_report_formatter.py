"""Token-Efficient LLM / AI Prompt Context Formatter for Gleam."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="gleam">',
        ...
        if patterns:
            lines.append("    <gleam_design_patterns_and_otp_actors>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <safety_and_architecture_hazards>")
            for v in hazards:
